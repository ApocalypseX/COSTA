            ...
            if self.is_focal:
                ...
                if self.wo_cost:
                    cost_whole_context=torch.cat([whole_sequence["observations"], whole_sequence["actions"], 
                                            whole_sequence["next_observations"]], dim=-1)
                else:
                    cost_whole_context=torch.cat([whole_sequence["observations"], whole_sequence["actions"], 
                ...
                if self.wo_cost:
                    ...
                    cost_first_context=torch.cat([first_sequence["observations"], first_sequence["actions"], 
                                                first_sequence["next_observations"]], dim=-1)
                else:
                    cost_whole_context=torch.cat([whole_sequence["observations"], whole_sequence["actions"], 
                                                whole_sequence["next_observations"], whole_sequence["costs"]], dim=-1)
                    cost_first_context=torch.cat([first_sequence["observations"], first_sequence["actions"], 
        ...
        for e in range(1, self._epoch + 1):
            ...
            for it in pbar:
                ...
                for task in range(self.num_tasks):
                    ...
                    with torch.no_grad():
                        context, context_first_sequence = self.sample_context((task-1)%self.num_tasks, train=True)
                        context_encoding = self.encoder(context, context_first_sequence)
                        context_encoding = context_encoding.mean(0).reshape(1,context_encoding.shape[1])
                    loss = self.policy.learn(batch, context_encoding, task, e)
                ...
                if self.use_sequence_batch:
                    ...
                    batch = self.buffer[task].random_batch(self._batch_size)
                context_encoding=torch.zeros(1,16,dtype=torch.float32)
                loss = self.policy.learn(batch, context_encoding, -1, e)
                res = dict(list(res.items())+list(loss.items()))
    ...
    def _evaluate(self) -> Dict[str, List[float]]:
        ...
        for task in range(self.num_tasks):
            ...
            with torch.no_grad():
                ...
                context, context_first_sequence = self.sample_context((task-1)%self.num_tasks)
                context_encoding = self.encoder(context, context_first_sequence)
                context_encoding = context_encoding.mean(0).reshape(1,context_encoding.shape[1])
