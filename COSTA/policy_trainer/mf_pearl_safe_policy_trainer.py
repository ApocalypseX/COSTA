...
class MFPEARLSafePolicyTrainer:
    def __init__(
        self,
        policy: BasePolicy,
        eval_env_list: list,
        ...
        for e in range(1, self._epoch + 1):
            ...
            for it in pbar:
                ...
                for task in range(self.num_tasks):
                    ...
                    with torch.no_grad():
                        context = self.sample_context(task, train=True)
                    loss = self.policy.learn(batch, context, task, e)
    ...
    def get_encoding(self, context):
        mean, sigma_squared=self.policy.encoder(context)
    ...
    def _evaluate(self) -> Dict[str, List[float]]:
        ...
        for task in range(self.num_tasks):
            ...
            with torch.no_grad():
                ...
                context = self.sample_context(task)
                context_encoding = self.get_encoding(context)
