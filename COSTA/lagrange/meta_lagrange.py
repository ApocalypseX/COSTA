...
class MetaLagrange:
    ...
    def __init__(
        ...
        self.lagrangian_multiplier = torch.nn.Sequential(torch.nn.Linear(encode_dim,encode_dim), torch.nn.ReLU(), torch.nn.Linear(encode_dim,1)).to(device)
        ...
        #     lr=lambda_lr,
        # )
        self.lambda_optimizer=torch.optim.Adam(self.lagrangian_multiplier.parameters(), lr=lambda_lr)

    def compute_lambda_loss(self, mean_ep_cost: float, context) -> torch.Tensor:
        ...
        Returns:
            Penalty loss for Lagrange multiplier.
        """
        return -self.lagrangian_multiplier(context) * (mean_ep_cost - self.cost_limit)

    def update_lagrange_multiplier(self, Jc: float, context) -> None:
        ...
        """
        #l_context=context.copy().detach().to(device)
        self.lambda_optimizer.zero_grad()
        lambda_loss = self.compute_lambda_loss(Jc, context)
