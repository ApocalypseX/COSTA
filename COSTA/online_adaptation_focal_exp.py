...
def do_adaptation(_config):
    ...
    for goal in range(num_tasks-1,-1,-1):
        ...
        while num_episodes < rnd_trj_num:
            ...
            exp_context[goal].append(con)
            #temp_con.append(con)
            temp_con=exp_context[goal][-t_ep_l:]
            ep_cost+=cost
            if ep_cost>=300:
                terminal=True
            obs = next_obs
    ...
    for i in range(len(context[0])):
        ...
        for goal in range(num_tasks):
            ...
            if _config["wo_iid"] and not _config["wo_exp"]:
                ...
                z=encoder(context[goal][i].squeeze().unsqueeze(0).to(device)).cpu().detach().numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--policy_path', type=str, default="run/cheetah-vel-safe/focal_cpq/seed_2_timestamp_23-0730-010427/model/policy.pth")
    ...
    if args.policy_path==None or args.exp_policy_path==None:
        raise ValueError("Policy path or exploration policy path should be given!")
    config_dict["policy_path"]=args.policy_path
    config_dict["exp_policy_path"]=args.exp_policy_path
    if args.wo_iid and args.wo_exp:
        ...
        log_dirs = make_log_dirs(args.task, "online_adaptation_focal_expert", config_dict["seed"], config_dict)
