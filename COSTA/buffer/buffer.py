...
class ReplayBuffer:
    def __init__(
        ...
        self.costs = np.zeros((self._max_size, 1), dtype=np.float32)
    ...
    def add(
        ...
        self.costs[self._ptr] = np.array(cost).copy()
    ...
    def add_batch(
        ...
        self.costs[indexes] = np.array(costs).copy()
    ...
    def load_dataset(self, dataset: Dict[str, np.ndarray]) -> None:
        ...
        costs = np.array(dataset["costs"], dtype=np.float32).reshape(-1, 1)
    ...
    def sample(self, batch_size: int) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "costs": torch.tensor(self.costs[batch_indexes]).to(self.device)
        }
    ...
    def sample_all(self) -> Dict[str, np.ndarray]:
        return {
            ...
            "costs": self.costs[:self._size].copy()
        }