...
class EnsembleDynamicsSafe(BaseDynamics):
    def __init__(
        self,
        model: nn.Module,
        ...
        self.cost_threshold = cost_penalty_threshold
    ...
    def step(
        ...
        ensemble_samples_m = (mean).astype(np.float32)

        # choose one model from ensemble
        num_models, batch_size, _ = ensemble_samples.shape
        model_idxs = self.model.random_elite_idxs(batch_size)
        # temp = ensemble_samples[model_idxs,:,:]
        # samples = temp[:,np.arange(batch_size),:]
        samples = ensemble_samples[model_idxs, np.arange(batch_size)]
        samples_m =ensemble_samples_m[model_idxs, np.arange(batch_size)]
        
        next_obs = samples[..., :-2]
        reward = samples_m[..., -2:-1]
        cost = samples_m[..., -1:]
        cost = np.clip(cost, 0.0, 1.0)
        cost = np.where(cost<0.5, np.zeros_like(cost), np.ones_like(cost))
        print(obs.shape, action.shape, next_obs.shape)
        terminal = self.terminal_fn(obs, action, next_obs)
        info = {}
        info["raw_reward"] = reward

        if self._penalty_coef or self.cost_threshold:
            if self._uncertainty_mode == "aleatoric":
                penalty = np.amax(np.linalg.norm(std, axis=2), axis=0)
            elif self._uncertainty_mode == "pairwise-diff":
                next_obses_mean = mean[..., :-2]
            ...
            cost = np.where(penalty < self.cost_threshold, cost, np.ones_like(cost))
            info["penalty"] = penalty
        
        return next_obs, reward, cost, terminal, info
    ...
    def format_samples_for_training(self, data: Dict) -> Tuple[np.ndarray, np.ndarray]:
        ...
        inputs = np.concatenate((obss, actions), axis=-1, dtype=np.float32)
        targets = np.concatenate((delta_obss, rewards, costs), axis=-1, dtype=np.float32)
