...
class PEARLCPQPolicy(SACPolicy):
    def __init__(
        self,
        actor: nn.Module,
        critic1: nn.Module,
        ...
        self.encoder=enocder.to(device)
    ...
    def learn(self, batch: Dict, context, task, e=0) -> Dict[str, float]:
        ...
        mean, sigma_squared=self.encoder(context)
        z_params = [self._product_of_gaussians(m, s) for m, s in zip(torch.unbind(mean), torch.unbind(sigma_squared))]
        ...
        context = torch.stack(z)
        org_context=context.detach()
        ...
        # update actor, do not use actor losses to update aggregator
        obss_with_context=torch.cat([obss, context.detach()], dim=-1)
        next_obss_with_context=torch.cat([next_obss, context.detach()], dim=-1)
        ...
        # use critic losses to update aggregator
            ...
            with torch.no_grad():
                ...
                if self.use_state_augmentation:
                    stacked_old_obs = old_obss.unsqueeze(1) \
                        .repeat(1, self._num_repeat_actions, 1) \
                        .view(batch_size * self._num_repeat_actions, old_obss.shape[-1])
                    _, mu, logv = self.vae_list[task](sampled_actions, stacked_old_obs)
                else:
                    _, mu, logv = self.vae_list[task](sampled_actions, stacked_org_obs)
        ...
        prior = torch.distributions.Normal(torch.zeros(z_means.shape[-1]).to(device), torch.ones(z_means.shape[-1]).to(device))
        ...
        if self.use_safety_lagrange:
            if self.lagrange_type == "common":
                result[str(task)+"/safety_lagrange"] = self.lagrange.lagrangian_multiplier(org_context).item()
            elif self.lagrange_type == "pid":
                result[str(task)+"/safety_lagrange"] = self.lagrange.lagrangian_multiplier
            else:
                raise ValueError("Not implemented yet.")
