...
class CPQPolicy(SACPolicy):
    def __init__(
        ...
        vae: nn.Module, #vae should be pre-trained
    ...
    def learn(self, batch: Dict, e=0, context=None) -> Dict[str, float]:
        ...
        if self.policy_train == "sac":
            ...
            if self.use_safety_lagrange:
                qca = self.critic_c(obss, a)
                if self.lagrange_type == "common":
                    actor_loss = (self._alpha * log_probs - (torch.min(q1a, q2a) - self.lagrange.lagrangian_multiplier.item() * qca)).mean()
                    if e > self.warm_up_epoch:
                        self.lagrange.update_lagrange_multiplier(qca.mean().item())
                elif self.lagrange_type == "pid":
                    actor_loss = (self._alpha * log_probs - (torch.min(q1a, q2a) - self.lagrange.lagrangian_multiplier * qca)).mean()
                    if e > self.warm_up_epoch:
                        self.lagrange.pid_update(qca.mean().item())
                else:
            ...
            if self.use_safety_lagrange:
                qca = self.critic_c(obss, a)
                if self.lagrange_type == "common":
                    actor_loss = ( - (torch.min(q1a, q2a) - self.lagrange.lagrangian_multiplier.item() * qca)).mean()
                    if e > self.warm_up_epoch:
                        self.lagrange.update_lagrange_multiplier(qca.max().item())
                elif self.lagrange_type == "pid":
                    actor_loss = ( - (torch.min(q1a, q2a) - self.lagrange.lagrangian_multiplier * qca)).mean()
                    if e > self.warm_up_epoch:
                        self.lagrange.pid_update(qca.max().item())
                else:
                    raise ValueError("Not implemented yet.")
            else:
        ...
        if self._max_q_backup:
            with torch.no_grad():
                ...
                #cpq indicator
                target_Qc = self.critic_c_old(next_obss, next_actions)
                if self.use_state_augmentation:
        ...
        current_Qc = self.critic_c(obss, actions)
            ...
            with torch.no_grad():
                ...
                if self.use_state_augmentation:
                    stacked_old_obs = old_obss.unsqueeze(1) \
                        .repeat(1, self._num_repeat_actions, 1) \
                        .view(batch_size * self._num_repeat_actions, old_obss.shape[-1])
                    _, mu, logv = self.vae(sampled_actions, stacked_old_obs)
                else:
                    _, mu, logv = self.vae(sampled_actions, stacked_obs)
        ...
        result =  {
            ...
            "loss/critic_c": critic_c_loss.item(),
            "loss/origin_critic_c": origin_critic_c_loss.item(),
            "loss/conservative_c": conservative_loss_c.item(),
            "cost_critic/cost_batch": current_Qc.mean().item(),
            "cost_critic/cost_batch_max": current_Qc.max().item()
        }
        if self.use_safety_lagrange:
            result["cost_critic/cost_pi"] = qca.mean().item()
            result["cost_critic/cost_pi_max"] = qca.max().item()
            result["reward_critic/reward_pi_1"] = q1a.mean().item()
            result["reward_critic/reward_pi_2"] = q2a.mean().item()
        ...
        if self.train_cpq_alpha:
            result["loss/cpq_alpha"] = cpq_alpha_loss.item()
            result["cpq_alpha"] = cpq_alpha.item()
        if self.use_safety_lagrange:
            if self.lagrange_type == "common":
                result["safety_lagrange"] = self.lagrange.lagrangian_multiplier.item()
            elif self.lagrange_type == "pid":
                result["safety_lagrange"] = self.lagrange.lagrangian_multiplier
            else:
                raise ValueError("Not implemented yet.")
