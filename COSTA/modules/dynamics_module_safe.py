...
class EnsembleDynamicsSafeModel(nn.Module):
    def __init__(
        self,
        obs_dim: int,
        action_dim: int,
        ...
        self.output_cost_layer = EnsembleLinear(
            hidden_dims[-1],
            2 * (self._with_cost),
            num_ensemble,
            weight_decays[-1]
        ...
        self.register_parameter(
            "max_logvar",
            nn.Parameter(torch.ones(obs_dim + self._with_reward + self._with_cost) * 0.5, requires_grad=True)
        )
        self.register_parameter(
            "min_logvar",
            nn.Parameter(torch.ones(obs_dim + self._with_reward + self._with_cost) * -10, requires_grad=True)
        )
    ...
    def forward(self, obs_action: np.ndarray) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        mean_c, logvar_c = torch.chunk(self.sigmoid(self.output_cost_layer(output)), 2, dim=-1)
        mean = torch.cat((mean, mean_c), dim=-1)
        logvar = torch.cat((logvar,logvar_c), dim=-1)
