...
from nets import *
from OfflineMetaLearner import OfflineMetaLearner
...
def run(_config):
    ...
    decoder=Decoder(2*observation_dim+action_dim+encode_dim,_config["decoder_hidden_layers"],output_dim=1)
    #decoder=None
    if decoder is not None:
        optimizer = torch.optim.AdamW([{'params': encoder.parameters()},
                                        {'params': decoder.parameters()}
                                        ], lr=_config["learning_rate"], eps=1e-5)
    else:
    ...
    for task in range(num_tasks):
        ...
        cost_dynamic_models.append(temp_model)
    learner=OfflineMetaLearner(encoder,
                           decoder,
                           optimizer,
                           cost_dynamic_models,
                           num_tasks,
...
if __name__=="__main__":
    ...
    config_dict["context_params"]["path"]=os.path.join(config_dict["context_params"]["path"],args.task,"context_encoder",mkfile_time)
