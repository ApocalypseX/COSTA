...
@register_env('humanoid-circle')
class HumanoidCircleEnv(HumanoidEnv):

    def __init__(self, task={}, n_tasks=2, randomize_tasks=True):
        self.tasks = self.sample_tasks(n_tasks)
        self.reset_task(0)
        super(HumanoidCircleEnv, self).__init__()
    ...
    def step(self, action):
        ...
        xyvelocity = (pos_after - pos_before)/self.dt
        x_v, y_v = xyvelocity

        x_pos, y_pos = pos_after[0], pos_after[1]
        d_xy = np.linalg.norm(pos_after, ord=2)
        d_o = 10
        x_lim = 3

        cir_reward = (- x_v * y_pos + y_v * x_pos) / (1 + np.abs(d_xy - d_o))
        ...
        reward = cir_reward - quad_ctrl_cost - quad_impact_cost + alive_bonus
        qpos = self.sim.data.qpos
        done = bool((qpos[2] < 1.0) or (qpos[2] > 2.0))

        return self._get_obs(), reward, done, dict(reward_circle=cir_reward,
    ...
    def sample_tasks(self, num_tasks):
        ...
        directions = np.arange(num_tasks)
