...
@register_env('ant-walk-safe')
class AntWalkEnv(MultitaskAntEnv):

    def __init__(self, task={}, n_tasks=3, forward_backward=False, max_episode_steps=300, randomize_tasks=True, **kwargs):
        self.forward_backward = forward_backward
        super(AntWalkEnv, self).__init__(task, n_tasks, max_episode_steps, **kwargs)
    ...
    def step(self, action):
        ...
        torso_velocity = np.abs(torso_xyz_after) - np.abs(torso_xyz_before)
        forward_reward = 0.25 * np.sum(torso_velocity[:2]/self.dt)
        ...
        obj_cost=0.0
        if self._goal==0:
            if xposafter<=-1 or yposafter>=0.87*xposafter+3 or yposafter<=0.87*xposafter-3:
                obj_cost = 1.0
        elif self._goal==1:
            if xposafter<=-1 or yposafter>=-0.87*xposafter+3 or yposafter<=-0.87*xposafter-3:
                obj_cost = 1.0
        elif self._goal==2:
            if xposafter>=1 or yposafter>=4 or yposafter<=-4:
                obj_cost = 1.0
        else:
            assert False
