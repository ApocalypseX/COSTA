...
def do_adaptation(_config):
    ...
    # create policy
    encoder=torch.load("run/"+_config['env_name']+"/"+_config["meta_params"]["focal_path"])
    ...
    for goal in range(num_tasks):
        ...
        while num_episodes < _config["exp_trj_num"]+_config["rnd_trj_num"]+_config["bootstrap_trj_num"]:
            ...
            if terminal:
                ...
                z=encoder(torch.stack(exp_context[goal][-ep_l:]).squeeze().unsqueeze(0).to(device)).cpu().detach().numpy()
                #z=encoder(torch.stack(exp_context[goal][-900:]).squeeze().unsqueeze(0).to(device)).cpu().detach().numpy()
                temp_con=[]
                ep_reward=0
                ep_cost=0
    print(rw_ls, cs_ls)
    ...
    plt.title("Online adaptation reward of focal")
    plt.legend()
    ...
    plt.ylabel("cost")
    plt.xlabel("online trajectory num")
    plt.title("Online adaptation cost of focal")
    plt.legend()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--idaq', type=int, default=0)
    parser.add_argument('--ood', type=int, default=0)
    parser.add_argument('--policy_path', type=str, default="run/cheetah-vel-safe/focal_cpq/seed_4_timestamp_23-0730-042029/model/policy.pth")
    args = parser.parse_args()
    config_dict=read_json_dict("configs/"+args.task+".json")
    config_dict["idaq"]=args.idaq
    ...
    if args.ood:
        if args.idaq:
            log_dirs = make_log_dirs(args.task, "task_generalization_idaq", config_dict["seed"], config_dict)
        else:
            log_dirs = make_log_dirs(args.task, "task_generalization_focal", config_dict["seed"], config_dict)
    elif args.idaq==0:
        log_dirs = make_log_dirs(args.task, "online_adaptation_focal", config_dict["seed"], config_dict)
    else:
        log_dirs = make_log_dirs(args.task, "online_adaptation_focal_idaq", config_dict["seed"], config_dict)
