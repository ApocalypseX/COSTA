...
from policy_trainer import MFCORROSafePolicyTrainer
...
def train(_config):
    ...
    encoder_aggregator = MLPAttnEncoder(input_dim=_config["context_params"]["encode_dim"], hidden_dims=[64,64,32,16], output_dim=_config["context_params"]["encode_dim"])
    encoder_optim = torch.optim.Adam(encoder_aggregator.parameters(), lr=_config["critic_lr"])
    ...
    policy_trainer = MFCORROSafePolicyTrainer(
        policy=policy,
        encoder=encoder,
...
if __name__ == "__main__":
    ...
    log_dirs = make_log_dirs(args.task, "corro_cpq", config_dict["seed"], config_dict)
