...
from policy import CORROCPQPolicy
...
def do_adaptation(_config):
    ...
    # create policy
    encoder=torch.load("run/"+_config['env_name']+"/"+_config["meta_params"]["corro_path"])
    encoder_aggregator = MLPAttnEncoder(input_dim=16, hidden_dims=[64,64,32,16], output_dim=16)
    encoder_optim = torch.optim.Adam(encoder_aggregator.parameters(), lr=1e-3)

    policy = CORROCPQPolicy(
        actor,
        critic1,
        critic2,
        critic_c,
    ...
    for goal in range(num_tasks):
        ...
        while num_episodes < _config["exp_trj_num"]+_config["rnd_trj_num"]+_config["bootstrap_trj_num"]:
            ...
            if terminal:
                ...
                z=policy.encoder_aggregator(encoder(torch.stack(exp_context[goal][-ep_l:]).squeeze().unsqueeze(0).to(device), mean=False)).cpu().detach().numpy()
                temp_con=[]
                ep_reward=0
                ep_cost=0
    print(rw_ls, cs_ls)
    ...
    for i in range(len(context[0])):
        ...
        for goal in range(num_tasks):
            ...
            ep_reward=0
            ep_cost=0
            z=policy.encoder_aggregator(encoder(torch.stack(context[goal][i]).squeeze().unsqueeze(0).to(device), mean=False)).cpu().detach().numpy()
    ...
    plt.title("Online adaptation reward of corro")
    plt.legend()
    ...
    plt.ylabel("cost")
    plt.xlabel("online trajectory num")
    plt.title("Online adaptation cost of corro")
    plt.legend()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--policy_path', type=str, default="run/cheetah-vel-safe/corro_cpq/seed_1_timestamp_23-0730-134240/model/policy.pth")
    ...
    if args.ood:
        log_dirs = make_log_dirs(args.task, "task_generalization_corro", config_dict["seed"], config_dict)
    else:
        log_dirs = make_log_dirs(args.task, "online_adaptation_corro", config_dict["seed"], config_dict)
