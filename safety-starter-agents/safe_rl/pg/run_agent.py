...
def init_buffer(n_trj, data_dir, task_name):
    ...
    train_trj_paths += glob.glob(os.path.join(data_dir, task_name, "trj_eval*_epoch*.npy" ))
    ...
    np.savez(os.path.join(data_dir, task_name, "offline_buffer"), obs=np.array(obs_train_lst),
...
def run_polopt_agent(env_fn, 
    ...
    if is_meta:
        ...
        ac_kwargs['action_space'] = envs[0].action_space

        # Inputs to computation graph from environment spaces
        x_ph, a_ph = placeholders_from_spaces(envs[0].observation_space, envs[0].action_space)
    else:
        # Share information about action space with policy architecture
    ...
    #=========================================================================#
    #config=tf.ConfigProto(device_count={"CPU":12},inter_op_parallelism_threads=1,intra_op_parallelism_threads=1)
    sess = tf.Session()
    ...
    # Setup model saving
    logger.setup_tf_saver(sess, inputs={'x': x_ph}, outputs={'pi': pi, 'v': v, 'vc': vc, 'mu':pi_info['mu']})
    ...
    if is_meta:
        o, r, d, c, ep_ret, ep_cost, ep_len = envs[0].reset(), 0, False, 0, 0, 0, 0
    else:
    ...
    for epoch in range(epochs):
        ...
        # Save model
        if (epoch % save_freq == 0) or (epoch == epochs-1) or (epoch>=epochs-100):
            if is_meta:
                if epoch>=epochs-100 or ((epoch>=int(epochs/3)-10 and epoch<=int(epochs/3)+10)):
                    logger.save_state({'env': envs}, epoch)
                else:
                    logger.save_state({'env': envs}, None)
            else:
                if epoch>=epochs-100 or (epoch>=int(epochs/3)-10 and epoch<=int(epochs/3)+10):
        ...
        if save_data and epoch % data_save_freq==0 and epoch>data_save_start:
            if is_meta:
                raise ValueError("Not implemented yet.")
            else:
                for episode in range(traj_per_ep):
                    o, r, d, c, ep_ret, ep_len, ep_cost = env.reset(), 0, False, 0, 0, 0, 0
                    trj = []
                    terminal=False
                    while not terminal:
                        ...
                        o2, r, d, info = env.step(a)
                        c = info.get('cost', 0)
                        trj.append([o, a, r, c, o2])
                        o = o2
                        ep_ret += r
                        ep_cost += c
                        ep_len += 1
                        terminal = d or (ep_len == max_ep_len)
                    if not os.path.exists(os.path.join("offline_data",env_name)):
                        os.makedirs(os.path.join("offline_data",env_name))
                    np.save(os.path.join("offline_data",env_name,'trj_eval{}_epoch{}.npy'.format(episode, epoch)), np.array(trj))
                o, r, d, c, ep_ret, ep_len, ep_cost = env.reset(), 0, False, 0, 0, 0, 0
        ...
        if is_meta:
            logger.log_tabular('Goal', env._goal)
        ...
        # Show results!
        logger.dump_tabular()
    init_buffer(-1, "offline_data", env_name)
